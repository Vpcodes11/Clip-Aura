...
from app.config import AI_API_MAX_RETRIES, AI_API_TIMEOUT_SECONDS
...
Remember: reject any content that promotes violence, hate, harassment, explicit material, or illegal activity."""
    ...
    if failed_chunks:
        logger.warning(
            "Transcript analysis partial failure: %s/%s chunks failed provider=%s failed_chunks=%s",
            len(failed_chunks), total_chunks, provider, failed_chunks
        )
    if not all_clips and failed_chunks:
        raise RuntimeError(
            f"Transcript analysis failed: all {total_chunks} chunks failed for provider={provider}. "
            f"The AI service may be unavailable."
        )

    # Fallback: if no clips found but all chunks succeeded (empty/boring content), generate deterministic clips
    if not all_clips:
    ...
    try:
        ...
        if provider == 'groq' and ("rate_limit" in err_msg or "429" in err_msg or "rate limit" in err_msg):
            ...
            if OPENAI_API_KEY:
                ...
                logger.warning("Groq rate limit hit. Falling back to OpenAI GPT.")
                try:
